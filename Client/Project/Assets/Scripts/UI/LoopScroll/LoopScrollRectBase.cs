...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        Func<int, RectTransform> instantiateFunc;
        Action<Transform> destroyFunc;

        protected void ReturnObject(Transform go)
        {
            //Debug.Log("ReturnObjectAndSendMessage:" + go);
            ...
            {
                this.destroyFunc.Invoke(go);
            }


        }

        protected void ProvideData(Transform transform, int index)
        {
            var binder = transform.GetComponent<ScriptBehaviour>();
            binder.proxy.OnPooledGet(index + 1);
        }


        protected RectTransform InstantiatePrefab(int idx)
        {

            var rect = instantiateFunc.Invoke(idx + 1);
            //Debug.Log("InstantiatePrefab:" + rect);
            return rect;
        }

        public void Init(Func<int,RectTransform> instanciateCallback, Action<Transform> destroyCallback)
        {

            this.instantiateFunc = instanciateCallback;
            this.destroyFunc = destroyCallback;

        }


        public void Refresh(int count, bool isTop = true)
        {
        ...
        protected override void Awake()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                if (m_Content != null)
                {
                    Debug.Assert(GetAbsDimension(m_Content.pivot) == value, "m_Content.pivot is error", this);
                    Debug.Assert(GetAbsDimension(m_Content.anchorMin) == value, "m_Content.anchorMin is error", this);
                    Debug.Assert(GetAbsDimension(m_Content.anchorMax) == value, "m_Content.anchorMax is error", this);
                }
                if (direction == LoopScrollRectDirection.Vertical)
                    Debug.Assert(m_Vertical && !m_Horizontal, "当前为垂直方向模式，应取消水平方向移动", this);
                else
                    Debug.Assert(!m_Vertical && m_Horizontal, "当前为水平方向模式，应取消垂直方向移动", this);
            }
        }
#endif
        ...
        protected virtual void LateUpdate()
        {
            if (!m_Content)
                return;
            if (this.instantiateFunc == null || this.destroyFunc == null)
            {
                GameDebug.Log("未初始化对象池方法");
                return;
            }
