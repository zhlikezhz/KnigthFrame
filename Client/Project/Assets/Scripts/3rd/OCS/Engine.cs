...
namespace Joy.OCS
{
    public class Engine
    {
        ...
        UInt32 GenerateEID()
        {
            ...
            {
                m_iEID = 1;
                Joy.Debug.LogError($"eid over UInt32.MaxValue: {m_iEID}");
            }
        ...
        {
            Joy.Debug.Assert(inst != null, "inst is null");
            if (inst == null) return;
        ...
        {
            Joy.Debug.Assert(type != null, "param type is null");
            ...
            if (comp != null)
            {
                ...
                if (m_dicEntity2Components.TryGetValue(eid, out var compList))
                {
                    ...
                    {
                        Joy.Debug.LogError($"component init error: {ex.ToString()}");
                    }
                }
                else
                {
                    Joy.Debug.LogError($"invaild eid");
                }
            }
            else
            {
                Joy.Debug.LogError("param type need inheirt component");
            }
            return comp;
        }
        ...
        void DeleteComponent(Component inst)
        {
            ...
            {
                Joy.Debug.LogError($"delete component fail: {ex.ToString()}");
            }
        }
        ...
        public System AddSystem(Type type)
        {
            ...
            if (sys != null)
            {
                ...
                {
                    Joy.Debug.LogError($"system init fail: {type.Name}.Start()\n{ex.ToString()}");
                }
            }
            else
            {
        ...
        {
            Joy.Debug.Assert(inst != null, "param inst is null");
            if (inst == null) return;
            if (m_Systems.Remove(inst))
            {
                ...
                {
                    Joy.Debug.LogError($"system destroy fail: {inst.GetType().Name}.OnDestroy()\n{ex.ToString()}");
                }
            }
        }
    }
